...
export type IWorkoutPlanner = Record<(typeof dayColumns)[number], Record<"Workout", IWorkout>> & {
	user_id: string;
	startDate: string;
	endDate: string;
};
...
export const commentWorkout = async (userId: string, workoutId: string, comment: string, day: string) => {
	try {
...
export const changeWorkout = async (
	...
	return post(url, { user_id: userId, workout_id: workoutId, feedback: reason, day: day });
};
...
export const changeLikeWorkout = async (
	...
	return post(url, { user_id: userId, workout_id: workoutId, feedback: `The user liked ${workoutName}`, liked: liked });
};
